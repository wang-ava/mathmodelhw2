...
- `code/results/optional_metrics.md`：Optional (2)/(3) 的结果汇总表。
...
- Optional (2) 实现了 `Repairing Sparse Low-rank Texture` 的低秩 + DCT 稀疏双先验修复。
- Optional (3) 实现了 `TILT` 的 affine / projective 两种整形模式。
